...
public class AnsjAnalyzer extends Analyzer {
	...
	public static Tokenizer getTokenizer(Reader reader, Map<String, String> args) {
		...
		//if (StringUtil.isNotBlank(temp = args.get(DicLibrary.DEFAULT))) { //用户自定义词典
			...
			//	if (StringUtil.isBlank(split[i])) {
				//	continue;
			//	}
				forests[0] = DicLibrary.get(DicLibrary.DEFAULT);
		//	}
	...
	//System.out.println("kk "+args.get("cnm"));	
		//if (StringUtil.isNotBlank(temp = args.get("cnm"))) { //用户自定义词典
		//	String[] split = temp.split(",");
			filters = new ArrayList<StopRecognition>();
		//	for (String key : split) {
	//	System.out.println("stop:"+key);		
				StopRecognition stop = StopLibrary.get(StopLibrary.DEFAULT);
		//System.out.println("stopA:"+stop);		
				if (stop != null)
					filters.add(stop);
		//	}
	//	}
